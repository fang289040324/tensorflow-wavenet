...
def create_variable(name, shape):
    ...
    initializer = tf.contrib.layers.xavier_initializer_conv2d()  # tensorflow.contrib.layers.python.layers.initializers.py
...
class WaveNetModel(object):
    ...
    def _generator_conv(self, input_batch, state_batch, weights):
        ...
        # tdo generalize to filter_width > 2
    ...
    def _generator_causal_layer(self, input_batch, state_batch):
        with tf.name_scope('causal_layer'):
            weights_filter = self.variables['causal_layer']['filter']
            output = self._generator_conv(input_batch, state_batch, weights_filter)
        return output
    ...
    def predict_proba(self, waveform, name='wavenet'):
        ...
        with tf.name_scope(name):
            ...
            last = tf.slice(proba, [tf.shape(proba)[0] - 1, 0], [1, self.quantization_channels])
    ...
    def predict_proba_incremental(self, waveform, name='wavenet'):
        ...
        if self.filter_width > 2:
            raise NotImplementedError("Incremental generation does not support filter_width > 2.")
        if self.scalar_input:
    ...
    def loss(self, input_batch, l2_regularization_strength=None, name='wavenet'):
        '''Creates a WaveNet network and returns the autoencoding loss.
        The variables are all scoped to the given name.
        '''
        with tf.name_scope(name):
            # We mu-law encode and quantize the input audioform.
            input_batch = mu_law_encode(input_batch, self.quantization_channels)
            ...
            with tf.name_scope('loss'):
                # Shift original input left by one sample, which means that each output sample has to predict the next input sample.
